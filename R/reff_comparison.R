# # # # # # # # # # # # # # # # # # # # # # # # 
# Plot multiple TP trajectories on same axes
# 
# David Price
# Feb, 2021
# 
...
require(tidyverse)
require(lubridate)
source('~/Desktop/COVID19_Data/forecast_plot_functions.R')
...
reff1 <- read_csv(paste0("~/Desktop/COVID19_Data/r_eff_1_local_samples_",the.date,"_noVac.csv"),
                  col_types =cols(
                    .default = col_double(),
...
reff2 <- read_csv(paste0("~/Desktop/COVID19_Data/r_eff_1_local_samples_",the.date,"_Vac.csv"),
                  col_types =cols(
                    .default = col_double(),
                    date = col_date(format = ""),
                    state = col_character(),
...
end.date <- read_csv(paste0("~/Desktop/COVID19_Data/output_dates_",the.date,".csv")) %>% select(latest_reff_date) %>% pull() + 1
date.label.format <- "%e/%m"
n.week.labels.panel <- 4
n.week.ticks <- "1 weeks"
...
ggsave(paste0("~/Desktop/COVID19_Data/reff_figures/r_eff_1_local_novacc_vacc_",the.date,".png"), height = 10, width = 9, bg = "white")
